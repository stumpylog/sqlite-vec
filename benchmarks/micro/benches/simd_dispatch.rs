...
fn random_int8_vector(n: usize) -> Vec<i8> {
    let mut rng = rand::thread_rng();
    (0..n).map(|_| rng.gen()).collect()
}
...
fn bench_distance_l2_int8(c: &mut Criterion) {
    init_vec();
    let db = Connection::open_in_memory().unwrap();
    let a = random_int8_vector(1536);
    let b = random_int8_vector(1536);
    // vec_int8(blob) accepts raw i8 bytes; reinterpret as u8 for the SQL blob type
    let a_bytes: Vec<u8> = a.iter().map(|&v| v as u8).collect();
    let b_bytes: Vec<u8> = b.iter().map(|&v| v as u8).collect();
    let mut stmt = db
        .prepare("select vec_distance_l2(vec_int8(?), vec_int8(?))")
        .unwrap();

    let mut group = c.benchmark_group("distance");
    group.bench_function("l2_int8_d1536", |bench| {
        bench.iter(|| {
            let _: f64 = stmt
...
fn bench_distance_l1_f32(c: &mut Criterion) {
    ...
    let a = random_vector(1536);
    let b = random_vector(1536);
    let mut stmt = db.prepare("select vec_distance_l1(?, ?)").unwrap();

    let mut group = c.benchmark_group("distance");
    group.bench_function("l1_float_d1536", |bench| {
        bench.iter(|| {
            let _: f64 = stmt
                .query_row(rusqlite::params![a.as_bytes(), b.as_bytes()], |r| r.get(0))
                .unwrap();
...
fn bench_distance_l1_int8(c: &mut Criterion) {
    init_vec();
    let db = Connection::open_in_memory().unwrap();
    let a = random_int8_vector(1536);
    let b = random_int8_vector(1536);
    ...
    let b_bytes: Vec<u8> = b.iter().map(|&v| v as u8).collect();
    let mut stmt = db
        .prepare("select vec_distance_l1(vec_int8(?), vec_int8(?))")
        .unwrap();

    let mut group = c.benchmark_group("distance");
    group.bench_function("l1_int8_d1536", |bench| {
        bench.iter(|| {
            let _: f64 = stmt
                .query_row(rusqlite::params![a_bytes, b_bytes], |r| r.get(0))
                .unwrap();
